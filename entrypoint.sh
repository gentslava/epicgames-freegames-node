...
CRON_SCHEDULE=$(cat $TEMP_CONFIG | jq -r ".cronSchedule[]? // .cronSchedule")
...
if [ "$RUN_ONCE" = "false" ]; then
    if [ "$DISTRO" = "alpine" ]; then
        ...
        echo "$CRON_SCHEDULE" | while read CRON_LINE ; do
            echo "Setting cron schedule as ${CRON_LINE}"
            echo "${CRON_LINE} cd /usr/app && node /usr/app/dist/src/index.js" | crontab -
        done
        ...
        echo "$CRON_SCHEDULE" | while read CRON_LINE ; do
            echo "Setting cron schedule as ${CRON_LINE}"
            echo "${CRON_LINE} . /root/project_env.sh && cd /usr/app && flock -n /var/lock/epicgames.lock node /usr/app/dist/src/index.js > /proc/1/fd/1 2>/proc/1/fd/2" | crontab -
        done
        # Run the cron process. The container should halt here and wait for the schedule.
        cron -f
    fi
